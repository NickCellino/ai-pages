...
async function setupQianfanBridge() {
  const checkCompatBtn = document.getElementById('check-compat');
  const compatStatus = document.getElementById('compat-status');
  const compatOutput = document.getElementById('compat-output');

  const endpointEl = document.getElementById('endpoint');
  const tokenEl = document.getElementById('token');
  const promptEl = document.getElementById('prompt');
  const uploadEl = document.getElementById('upload');
  const runRemoteBtn = document.getElementById('run-remote');
  const remoteStatus = document.getElementById('remote-status');
  const remoteOutput = document.getElementById('remote-output');

  checkCompatBtn.addEventListener('click', async () => {
    compatStatus.classList.remove('good');
    compatStatus.textContent = 'Checking model metadata…';
    try {
      const res = await fetch('https://huggingface.co/api/models/baidu/Qianfan-OCR');
      if (!res.ok) throw new Error(`Model API HTTP ${res.status}`);
      const model = await res.json();

      const tags = model.tags || [];
      const hasTransformersJsTag = tags.includes('transformers.js');
      const hasOnnx = (model.siblings || []).some((s) => String(s.rfilename || '').includes('onnx'));

      const report = {
        modelId: model.id,
        pipeline_tag: model.pipeline_tag,
        library_name: model.library_name,
        safetensor_params: model.safetensors?.parameters?.F16 || model.safetensors?.parameters || null,
        has_transformers_js_tag: hasTransformersJsTag,
        has_onnx_artifacts: hasOnnx,
        summary:
          hasTransformersJsTag || hasOnnx
            ? 'Some browser-friendly signals found; still may require architecture support.'
            : 'No transformers.js tag and no ONNX artifacts detected. Direct in-browser use is unlikely right now.',
      };

      compatOutput.textContent = JSON.stringify(report, null, 2);
      compatStatus.classList.add('good');
      compatStatus.textContent = 'Compatibility check complete.';
    } catch (err) {
      compatStatus.textContent = `Check failed: ${err.message}`;
      compatOutput.textContent = 'Unable to fetch model metadata.';
    }
  });

  runRemoteBtn.addEventListener('click', async () => {
    const [file] = uploadEl.files || [];
    if (!file) {
      remoteStatus.textContent = 'Please upload an image first.';
      return;
    }

    remoteStatus.classList.remove('good');
    remoteStatus.textContent = 'Sending request to remote endpoint…';
    remoteOutput.textContent = 'Waiting for response...';
    ...
    try {
      ...
      // Most image-to-text inference endpoints accept raw image bytes.
      // Prompt support varies by provider; this demo sends it as a best-effort header.
      if (promptEl.value.trim()) {
        headers['X-Prompt'] = promptEl.value.trim();
      }
      ...
      if (!response.ok) {
        remoteOutput.textContent = JSON.stringify({
          status: response.status,
          statusText: response.statusText,
          response: parsed,
        }, null, 2);
        remoteStatus.textContent = 'Remote endpoint returned an error (see output).';
        return;
      }

      remoteOutput.textContent = typeof parsed === 'string' ? parsed : JSON.stringify(parsed, null, 2);
      remoteStatus.classList.add('good');
      remoteStatus.textContent = 'Remote request completed.';
    } catch (err) {
      remoteStatus.textContent = `Remote test failed: ${err.message}`;
      remoteOutput.textContent = 'Request could not be completed. Check endpoint CORS/auth/payload format.';
    }
  });
}
